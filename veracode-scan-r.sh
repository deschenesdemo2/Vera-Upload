#/bin/bash
        ...
        if `wget https://repo1.maven.org/maven2/com/veracode/vosp/api/wrappers/vosp-api-wrappers-java/20.3.6.1/vosp-api-wrappers-java-20.3.6.1.jar -O VeracodeJavaAPI.jar`; then
        ...
        java -jar $JAVA_WRAPPER_LOCATION/VeracodeJavaAPI.jar -vid $1 -vkey $2 -action uploadandscan -appname $3 -createprofile false -filepath $4 -version $VERSION > $OUTPUT_FILE_FULL_PATH 2>&1
        echo ""
        ...
        #Get Build ID
        build_id=$(cat $OUTPUT_FILE_FULL_PATH | grep build_id | awk -F "\"" '{print $2}')
        echo ""
        echo '====== DEBUG START ======'
        ...
        while true;
        do
             java -jar $JAVA_WRAPPER_LOCATION/VeracodeJavaAPI.jar -vid $1 -vkey $2 -action getbuildinfo -appid $app_ID > $OUTPUT_FILE_FULL_PATH 2>&1
             scan_status=$(cat $OUTPUT_FILE_FULL_PATH)
             ...
             then
                   java -jar $JAVA_WRAPPER_LOCATION/VeracodeJavaAPI.jar -vid $1 -vkey $2 -action GetPreScanResults -appid $app_ID > $OUTPUT_FILE_FULL_PATH 2>&1
                   echo ""
                   echo '[ ERROR ] Something went wrong with the prescan!'
                   cat $OUTPUT_FILE_FULL_PATH
                   rm -rf $OUTPUT_FILE_FULL_PATH
                   exit 1 && break
             else
                   scan_finished=$(cat $OUTPUT_FILE_FULL_PATH)
        ...
        java -jar $JAVA_WRAPPER_LOCATION/VeracodeJavaAPI.jar -vid $1 -vkey $2 -action passfail -appname $3 > $OUTPUT_FILE_FULL_PATH 2>&1
        scan_result=$(cat $OUTPUT_FILE_FULL_PATH)
